...
int main(int argc, char **argv)
{
    ...
    ros::Publisher pub = nodeHandler.advertise<geometry_msgs::Pose>("mono/pose", 10);
...
void ImageGrabber::GrabImage(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    cam_msg.position.x = trans_mat.at<float>(0, 3);
    cam_msg.position.y = trans_mat.at<float>(1, 3);
    cam_msg.position.z = trans_mat.at<float>(2, 3);
    mpub->publish(cam_msg);
